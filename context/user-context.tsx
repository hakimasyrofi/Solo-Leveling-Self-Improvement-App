...
interface UserContextType {
  ...
  addCustomQuest: (quest: Omit<Quest, "id" | "active" | "completed" | "progress">) => void
...
export function UserProvider({ children }: { children: React.ReactNode }) {
    ...
    setUserStats((prevStats) => {
      ...
      newStats = {
        ...
        quests: newStats.quests.map((q) =>
          q.id === questId ? { ...q, progress: 100, completed: true, active: false } : q,
        ),
      }
  ...
  const updateQuestProgress = (questId: string, progress: number) => {
    setUserStats((prevStats) => ({
      ...prevStats,
      quests: prevStats.quests.map((quest) => (quest.id === questId ? { ...quest, progress } : quest)),
    }))
  }

  // Add a custom quest
  const addCustomQuest = (quest: Omit<Quest, "id" | "active" | "completed" | "progress">) => {
    const newQuest = {
      ...quest,
      id: uuidv4(),
      active: true,
  ...
  const removeItem = (itemId: string, quantity?: number) => {
    setUserStats((prevStats) => removeItemFromInventory(prevStats, itemId, quantity))
  }
